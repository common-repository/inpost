<?php
/**
 * Plugin Name: InPost WooCommerce
...
class My_Example_List_Table extends WP_List_Table
{
	...
	private function update_parcel_details($id, $parcel_detail)
	{
		...
		// We are going to put a link into the file_name field which
		// contains a program which will create the label when the
		...
		$parcel_id   = $parcel_detail->id;

		$create_label = plugins_url() . '/inpost/create_label.php';

		$sql_data = array(
			'parcel_detail' => json_encode($parcel_detail),
			'file_name'     => '<a href="' . $create_label .
				'?api_url=' . $url .
				'&parcel_id=' . $parcel_id .
				'&api_key=' . $key .
				'&format=' . $label_type .
	...
	function get_columns()
	{
		$columns = array(
			...
			'parcel_target_machine_id' => __('Locker ID', 'inpost'),
			'sticker_creation_date'    => __('Label creation date', 'inpost'),

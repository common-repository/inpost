...
class WC_InPostShippingMethod extends WC_Shipping_Method
{
	...
	*/
	function init()
	{
		...
		$this->shippment    = $this->get_option( 'shippment' );
	...
	*/
	function init_form_fields()
	{
		...
		'shipment' => array(
			'title'       => __( 'Shipment Override', 'inpostplugin' ),
			'type'        => 'checkbox',
			'description' => __( 'The user shipment address is replaced with InPost Locker, if selected.', 'inpostplugin' ),
			'default'     => __( 'false', 'inpostplugin' ),
			'desc_tip'    => true,
		),
		'label_type' => array(
	...
	function add_my_js()
	{
		...
		if(is_checkout())
		{
			...
			if($ret == true)
			{
				echo '<input type="hidden" name="inpost_can_use" id="inpost_can_use" value="1" readonly>' . "\n";
			}
			else
			{
				echo '<input type="hidden" name="inpost_can_use" id="inpost_can_use" value="0" readonly>' . "\n";
			}

			// Check if the user's shipment address should be
			// overriden or not?
			$config_data = get_option('woocommerce_inpost_shipping_method_settings');
			$shipment = strtolower(trim($config_data['shipment']));

			if (strcasecmp($shipment, 'yes') == 0)
			{
				echo '<input type="hidden" name="inpost_shipment" id="inpost_shipment" value="1" readonly>' . "\n";
			}
			else
			{
				echo '<input type="hidden" name="inpost_shipment" id="inpost_shipment" value="0" readonly>' . "\n";
			}
		}
